...
# only want to call if the direction we are going towards is unexplored
def move_to_room_and_add(direction):
    traversalPath.append(direction)
    prev_room = player.currentRoom.id
    player.travel(direction)
    discovered_room = player.currentRoom.id
    visited_rooms.add(player.currentRoom)
    populate_traversal()
    traversal[prev_room][direction], traversal[discovered_room][reverse_direction[direction]] = discovered_room, prev_room

def get_path_to_unexplored():
    visited = set()
    visited.add(player.currentRoom.id)
    stack = [[direction] for direction in traversal[player.currentRoom.id]]
    found = False
    while len(stack) > 0 and found == False:
        path = stack.pop()
        cur_room = traversal[player.currentRoom.id][path[0]]
        for j in range(1, len(path)):
            cur_room = traversal[cur_room][path[j]]
        if cur_room not in visited:
            visited.add(cur_room)
            for next_room in traversal[cur_room]:
                if traversal[cur_room][next_room] == '?':
                    found = True
                    return [* path, next_room]
                stack.insert(0, [*path, next_room])
...
populate_traversal()
move_to_room_and_add('e')

while len(visited_rooms) < len(roomGraph):
    next_direction = None
    for possible_direction in traversal[player.currentRoom.id]:
        if traversal[player.currentRoom.id][possible_direction] == '?':
            next_direction = possible_direction
            move_to_room_and_add(next_direction)
            break
    if next_direction == None:
        path = get_path_to_unexplored()
        if path == None:
            break
        for i in range(len(path) - 1):
            player.travel(path[i])
            traversalPath.append(path[i])
        move_to_room_and_add(path[-1])
